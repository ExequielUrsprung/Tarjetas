...
.    */
t  public static string GetTokenName( int token) {
...
t            if (debug != null)
t              debug.lex(yyState, yyToken, GetTokenName( yyToken), yyLex.value());
.          }
...
.              }
t              if (debug != null)
t                debug.discard(yyState, yyToken, GetTokenName( yyToken),
t  							yyLex.value());
.              yyToken = -1;
