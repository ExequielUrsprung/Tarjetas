    ...
    public abstract class StringPaddingLeft : IStringPadding
    {
        ...
        public virtual string Pad(string data, int totalWidth)
        {
            ...
            if (!_truncate && (data.Length > totalWidth))
                ...
                throw new ArgumentException("Unexpected bigger data length.", "data");

            if (data.Length == totalWidth) // No string padding necessary, reduce overhead returning
                // same value.
                return data;

            return data.Length > totalWidth ? data.Substring(0, totalWidth) : data.PadLeft(totalWidth, _padChars[0]);
        }
        ...
        public virtual string RemovePad(string data)
        {
            ...
            return data[0] == _padChars[0] ? data.TrimStart(_padChars) : data;
        }
        #endregion
    }
}